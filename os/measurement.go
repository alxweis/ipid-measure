...
func Run(c *config.OSConfig, m *paths.OSMeasurement) (uint64, error) {
	...
	log.Printf("dns_chaos_workers      = %s", c.DNSChaosWorkers.String())
	...
	log.Printf("coverage_path          = %s", m.CoverageFilePath)
	log.Printf("====================================")
	...
	return runPipeline(ctx, c, m.ZMapLinkPath, m.MeasurementFilePath, m.CoverageFilePath)
}
